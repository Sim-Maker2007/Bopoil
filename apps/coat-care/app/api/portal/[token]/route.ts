...
import { appointmentCareRecords, appointmentChangeClaims, appointmentReservations, appointments, auditEvents, clientMediaAssets, clientPhoneIdentities, clientPhoneOtpChallenges, clientPortalSessions, clients, invoices, locations, onlinePaymentSessions, organizations, pets, salonSettings, services, staff, vaccinationRecords } from "../../../../db/schema";
...
async function portalPayload(token: string) {
  ...
  const [[organization], petRows, appointmentRows, vaccines, openDepositRows, sharedPhotoRows] = await Promise.all([
    ...
    db.select({ id: clientMediaAssets.id, kind: clientMediaAssets.kind, caption: clientMediaAssets.caption, originalFilename: clientMediaAssets.originalFilename, createdAt: clientMediaAssets.createdAt }).from(clientMediaAssets).where(and(eq(clientMediaAssets.organizationId, client.organizationId), eq(clientMediaAssets.clientId, client.id), eq(clientMediaAssets.clientVisible, true))).orderBy(desc(clientMediaAssets.createdAt)),
  ]);
  const now = Date.now();
