  ...
  return <main className="app-shell">
    ...
    <div className="client-view">
      <section className="hero-section"><Image className="hero-image" src="/grooming-hero.jpg" alt="A freshly groomed poodle in a warm, modern salon" fill priority unoptimized sizes="(max-width: 760px) 100vw, 94vw"/><div className="hero-shade"/><div className="hero-content"><span className="eyebrow light">Thoughtful grooming, beautifully simple</span><h1>A happier grooming day<br/>starts right here.</h1><p>Book trusted care tailored to your pet—without the back-and-forth.</p></div>
        <form className={`search-panel ${catalog && catalog.locations.length > 1 ? "with-location" : ""}`} aria-label="Find an appointment" onSubmit={(event) => { event.preventDefault(); if (squareManaged && !bookingContext) { window.location.assign(`/fiche-informations.html?continue=${encodeURIComponent(currentBookingReturnTo)}`); return; } if (serviceId && onlineBookingOpen) chooseService(serviceId); }}>{catalog && catalog.locations.length > 1 && <label><span>Location</span><select value={catalog.location.slug} onChange={(event) => window.location.assign(bookingContext ? bookingUrl(catalog.organization.slug, event.target.value, selectedPetId, serviceId) : `/book/${encodeURIComponent(catalog.organization.slug)}/${encodeURIComponent(event.target.value)}`)}>{catalog.locations.map((item) => <option value={item.slug} key={item.slug}>{item.name} · {item.city}</option>)}</select></label>}<label><span>{bookingContext?.pets.length ? "Pet" : squareManaged ? "BOPOIL profile" : "Pet name"}</span>{bookingContext?.pets.length ? <select value={selectedPetId} onChange={(event) => selectOwnedPet(event.target.value)}>{bookingContext.pets.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.breed}</option>)}</select> : <input value={pet} onChange={(event) => setPet(event.target.value)} placeholder={squareManaged ? "Create or open profile" : "Your pet’s name"} autoComplete="off" readOnly={squareManaged}/>}</label><label><span>Service</span><select value={serviceId} onChange={(event) => { setServiceId(event.target.value); if (onlineBookingOpen) void loadAvailability(event.target.value); }}>{catalog?.services.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label><span>When</span><select value={selectedDate} onChange={(event) => chooseDate(event.target.value)} disabled={!onlineBookingOpen || availabilityLoading || (squareManaged && !bookingContext)}>{availability?.dates.filter((item) => item.slots.length).map((item) => <option key={item.date} value={item.date}>{dayLabel(item.date)}</option>)}</select></label><button type="submit" className="search-button" disabled={!serviceId || availabilityLoading || !onlineBookingOpen || (!selectedDate && !(squareManaged && !bookingContext))}>{squareManaged && !bookingContext ? "Create profile" : onlineBookingOpen ? availabilityLoading ? "Checking…" : "Find a time" : "Online booking paused"} <span aria-hidden="true">→</span></button></form>
      ...
      <section className="booking-flow" id="booking"><div className="booking-intro"><span className="eyebrow">Booking made calm</span><h2>{step === "confirmed" ? bookingStatus === "requested" ? "Request received." : "You’re all set." : "Let’s plan their visit."}</h2><p>{step === "confirmed" ? bookingStatus === "requested" ? `The team at ${catalog?.location.name || "the salon"} will review ${pet}’s time and confirm shortly.` : `Everything ${pet} needs is saved and ready for the salon.` : "Every opening below is checked against team skills, working hours, equipment, and pets already in care."}</p><div className="step-track" aria-label={`Step ${stepNumber} of 4`}>{[1,2,3,4].map((number) => <span key={number} className={number <= stepNumber ? "done" : ""}/>)}</div></div>
        <div className="booking-card" ref={bookingCard}>
          ...
          {step === "search" && contextChecked && clientAuthStep === "closed" && !bookingContext && <div className="empty-booking"><span className="pet-medallion">♡</span><h3>{squareManaged ? "Let’s create their permanent profile." : "Ready when they are."}</h3><p>{squareManaged ? "Your BOPOIL profile keeps every pet, health note, and preference together while Square handles the appointment calendar." : "Browse tailored services and genuine availability, or securely bring back your saved pets."}</p><button className="primary-button" onClick={() => squareManaged ? window.location.assign(`/fiche-informations.html?continue=${encodeURIComponent(currentBookingReturnTo)}`) : openServices()}>{squareManaged ? "Create profile & add pet" : "Start a new booking"}</button>{smsDeliveryConfigured ? <button className="returning-link" onClick={() => openClientAuth("signin")}>Already a client? Continue with mobile →</button> : <button className="returning-link" onClick={openEmailAccess}>Already a client? Use email →</button>}{isDevelopmentPreview && <button className="returning-link" disabled={demoReturningBusy} onClick={() => void previewReturningCustomer()}>{demoReturningBusy ? "Opening returning-client preview…" : "Preview returning client (local only) →"}</button>}</div>}
