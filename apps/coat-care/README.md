...
Square remains the scheduling and terminal source of truth while Coat & Care owns client identity, pet records, health information, preferences, and day-of-care work. Configure `SQUARE_ACCESS_TOKEN`, `SQUARE_WEBHOOK_SIGNATURE_KEY`, `SQUARE_LOCATION_ID`, `SQUARE_ORGANIZATION_SLUG`, and `SQUARE_LOCATION_SLUG`, then register `booking.created` and `booking.updated` at `/api/webhooks/square`. Set `SQUARE_BOOKING_MODE=api` for an Appointments Plus/Premium account after granting the token Customers, Items, Team, and Appointments read/write permissions. The website then searches Square availability and creates every booking with the permanently linked Square customer ID. Set `SQUARE_WEBHOOK_NOTIFICATION_URL` when the registered URL differs from `DELIVERY_PUBLIC_URL + /api/webhooks/square`; webhook signatures depend on an exact URL match. The API version defaults to `2026-07-15` and can be overridden with `SQUARE_API_VERSION`.

Square appointments are imported without duplicate Coat & Care confirmations or reminders. Scheduling changes remain in Square; Coat & Care keeps operational stages, care notes, approvals, photos, and report cards. Incoming Square customer data can seed an unknown profile but never overwrites an existing Coat & Care client. Ambiguous legacy duplicates stop for staff review instead of being merged automatically. An hourly reconciliation recovers missed webhook deliveries. The BOPOIL information form posts to `/api/public/intake`; restrict it with `PUBLIC_INTAKE_ALLOWED_ORIGINS`.
