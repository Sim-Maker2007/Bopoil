...
export const organizations = pgTable("organizations", {
  ...
  updatedAt: text("updated_at").notNull().default(sql`to_char(now() at time zone 'utc', 'YYYY-MM-DD HH24:MI:SS')`),
});
...
export const salonSettings = pgTable("salon_settings", {
  ...
  requireOnlineDeposit: boolean("require_online_deposit").notNull().default(false),
...
export const portalAccessRequests = pgTable("portal_access_requests", {
  ...
  requestedAt: text("requested_at").notNull().default(sql`to_char(now() at time zone 'utc', 'YYYY-MM-DD HH24:MI:SS')`),
}, (table) => [index("portal_access_requests_email_time_idx").on(table.emailHash, table.requestedAt)]);

export const clientPhoneIdentities = pgTable("client_phone_identities", {
  id: text("id").primaryKey(),
  organizationId: text("organization_id").notNull().references(() => organizations.id),
  clientId: text("client_id").notNull().references(() => clients.id),
...
export const clientPhoneOtpChallenges = pgTable("client_phone_otp_challenges", {
  id: text("id").primaryKey(),
  organizationId: text("organization_id").notNull().references(() => organizations.id),
  phoneE164: text("phone_e164").notNull(),
...
export const petCareProfiles = pgTable("pet_care_profiles", {
  id: text("id").primaryKey(),
  organizationId: text("organization_id").notNull().references(() => organizations.id),
  ...
  treatsAllowed: boolean("treats_allowed"),
  marketingPhotosAllowed: boolean("marketing_photos_allowed"),
...
export const publicIntakeSubmissions = pgTable("public_intake_submissions", {
  id: text("id").primaryKey(),
  organizationId: text("organization_id").notNull().references(() => organizations.id),
  locationId: text("location_id").notNull().references(() => locations.id),
...
export const externalEntityLinks = pgTable("external_entity_links", {
  ...
  lastSyncedAt: text("last_synced_at").notNull().default(sql`to_char(now() at time zone 'utc', 'YYYY-MM-DD HH24:MI:SS')`),
  createdAt: text("created_at").notNull().default(sql`to_char(now() at time zone 'utc', 'YYYY-MM-DD HH24:MI:SS')`),
}, (table) => [
...
export const integrationSyncStates = pgTable("integration_sync_states", {
  id: text("id").primaryKey(),
  organizationId: text("organization_id").notNull().references(() => organizations.id),
  locationId: text("location_id").notNull().references(() => locations.id),
  provider: text("provider", { enum: ["square"] }).notNull(),
...
export const staffServiceSkills = pgTable("staff_service_skills", {
  id: text("id").primaryKey(),
  organizationId: text("organization_id").notNull().references(() => organizations.id),
  locationId: text("location_id").notNull().references(() => locations.id),
  staffId: text("staff_id").notNull().references(() => staff.id),
...
export const appointmentReservations = pgTable("appointment_reservations", {
  id: text("id").primaryKey(),
  organizationId: text("organization_id").notNull().references(() => organizations.id),
  locationId: text("location_id").notNull().references(() => locations.id),
...
export const appointmentChangeClaims = pgTable("appointment_change_claims", {
  id: text("id").primaryKey(),
  organizationId: text("organization_id").notNull().references(() => organizations.id),
  appointmentId: text("appointment_id").notNull().references(() => appointments.id),
...
export const waitlistConversionClaims = pgTable("waitlist_conversion_claims", {
  id: text("id").primaryKey(),
  organizationId: text("organization_id").notNull().references(() => organizations.id),
...
export const paymentEvents = pgTable("payment_events", {
  ...
  occurredAt: text("occurred_at").notNull().default(sql`to_char(now() at time zone 'utc', 'YYYY-MM-DD HH24:MI:SS')`),
  createdAt: text("created_at").notNull().default(sql`to_char(now() at time zone 'utc', 'YYYY-MM-DD HH24:MI:SS')`),
}, (table) => [
...
export const invoiceMutationClaims = pgTable("invoice_mutation_claims", {
  id: text("id").primaryKey(),
  organizationId: text("organization_id").notNull().references(() => organizations.id),
  invoiceId: text("invoice_id").notNull().references(() => invoices.id),
...
export const expenses = pgTable("expenses", {
  ...
  recoverableTax: boolean("recoverable_tax").notNull().default(false),
...
export const financialAccountTransactions = pgTable("financial_account_transactions", {
  id: text("id").primaryKey(),
  organizationId: text("organization_id").notNull().references(() => organizations.id),
  locationId: text("location_id").references(() => locations.id),
...
export const purchaseOrderClaims = pgTable("purchase_order_claims", {
  id: text("id").primaryKey(),
  organizationId: text("organization_id").notNull().references(() => organizations.id),
  locationId: text("location_id").notNull().references(() => locations.id),
  purchaseOrderId: text("purchase_order_id").notNull().references(() => purchaseOrders.id),
...
export const inventoryMovementClaims = pgTable("inventory_movement_claims", {
  id: text("id").primaryKey(),
  organizationId: text("organization_id").notNull().references(() => organizations.id),
  locationId: text("location_id").notNull().references(() => locations.id),
  inventoryItemId: text("inventory_item_id").notNull().references(() => inventoryItems.id),
...
export const messages = pgTable("messages", {
  ...
  scheduledFor: text("scheduled_for").notNull().default(sql`to_char(now() at time zone 'utc', 'YYYY-MM-DD HH24:MI:SS')`),
  sentAt: text("sent_at"),
...
export const deliveryProviderEvents = pgTable("delivery_provider_events", {
  ...
  error: text("error").notNull().default(""),
  receivedAt: text("received_at").notNull().default(sql`to_char(now() at time zone 'utc', 'YYYY-MM-DD HH24:MI:SS')`),
  processedAt: text("processed_at"),
}, (table) => [
...
export const petWarnings = pgTable("pet_warnings", {
  id: text("id").primaryKey(),
  organizationId: text("organization_id").notNull().references(() => organizations.id),
  petId: text("pet_id").notNull().references(() => pets.id),
...
export const appointmentCareRecords = pgTable("appointment_care_records", {
  id: text("id").primaryKey(),
  organizationId: text("organization_id").notNull().references(() => organizations.id),
  locationId: text("location_id").notNull().references(() => locations.id),
  appointmentId: text("appointment_id").notNull().references(() => appointments.id),
  ...
  reportPublished: boolean("report_published").notNull().default(false),
...
export const timeEntries = pgTable("time_entries", {
  ...
  createdAt: text("created_at").notNull().default(sql`to_char(now() at time zone 'utc', 'YYYY-MM-DD HH24:MI:SS')`), updatedAt: text("updated_at").notNull().default(sql`to_char(now() at time zone 'utc', 'YYYY-MM-DD HH24:MI:SS')`),
...
export const staffClockStates = pgTable("staff_clock_states", {
  id: text("id").primaryKey(), organizationId: text("organization_id").notNull().references(() => organizations.id),
  locationId: text("location_id").notNull().references(() => locations.id), staffId: text("staff_id").notNull().references(() => staff.id),
  ...
  version: integer("version").notNull().default(0), updatedAt: text("updated_at").notNull().default(sql`to_char(now() at time zone 'utc', 'YYYY-MM-DD HH24:MI:SS')`),
}, (table) => [uniqueIndex("staff_clock_state_unique").on(table.locationId, table.staffId)]);

export const timeClockClaims = pgTable("time_clock_claims", {
  id: text("id").primaryKey(), organizationId: text("organization_id").notNull().references(() => organizations.id),
  locationId: text("location_id").notNull().references(() => locations.id), staffId: text("staff_id").notNull().references(() => staff.id),
  expectedVersion: integer("expected_version").notNull(), action: text("action", { enum: ["clock_in", "clock_out"] }).notNull(),
  timeEntryId: text("time_entry_id").notNull().references(() => timeEntries.id), createdAt: text("created_at").notNull().default(sql`to_char(now() at time zone 'utc', 'YYYY-MM-DD HH24:MI:SS')`),
}, (table) => [uniqueIndex("time_clock_claim_staff_version_unique").on(table.locationId, table.staffId, table.expectedVersion)]);

export const employeePortalCredentials = pgTable("employee_portal_credentials", {
  id: text("id").primaryKey(), organizationId: text("organization_id").notNull().references(() => organizations.id),
  ...
  lockedUntil: text("locked_until"), active: boolean("active").notNull().default(true),
  createdAt: text("created_at").notNull().default(sql`to_char(now() at time zone 'utc', 'YYYY-MM-DD HH24:MI:SS')`), updatedAt: text("updated_at").notNull().default(sql`to_char(now() at time zone 'utc', 'YYYY-MM-DD HH24:MI:SS')`),
...
export const employeePortalInvitations = pgTable("employee_portal_invitations", {
  id: text("id").primaryKey(), organizationId: text("organization_id").notNull().references(() => organizations.id),
  ...
  invitedByStaffId: text("invited_by_staff_id").notNull().references(() => staff.id), createdAt: text("created_at").notNull().default(sql`to_char(now() at time zone 'utc', 'YYYY-MM-DD HH24:MI:SS')`),
...
export const payrollPeriods = pgTable("payroll_periods", {
  ...
  createdByStaffId: text("created_by_staff_id").notNull().references(() => staff.id), createdAt: text("created_at").notNull().default(sql`to_char(now() at time zone 'utc', 'YYYY-MM-DD HH24:MI:SS')`), updatedAt: text("updated_at").notNull().default(sql`to_char(now() at time zone 'utc', 'YYYY-MM-DD HH24:MI:SS')`),
...
export const payrollLines = pgTable("payroll_lines", {
  ...
  compensationSnapshotJson: text("compensation_snapshot_json").notNull().default("{}"), createdAt: text("created_at").notNull().default(sql`to_char(now() at time zone 'utc', 'YYYY-MM-DD HH24:MI:SS')`),
